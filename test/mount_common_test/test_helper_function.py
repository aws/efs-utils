...
import efs_utils_common.config_utils as config_utils
...
import efs_utils_common.process_utils as process_utils
...
def test_is_instance_metadata_url_helper():
    assert False == metadata.is_instance_metadata_url(
        efs_utils_common.constants.ECS_TASK_METADATA_API
    )
    assert True == metadata.is_instance_metadata_url(
...
def _test_get_boolean_config_item_in_config_file_helper(
    ...
    assert expected_value == config_utils.get_boolean_config_item_value(
...
def test_get_true_boolean_config_item_in_config_file():
    ...
    assert True == config_utils.get_boolean_config_item_value(
...
def test_get_false_boolean_config_item_in_config_file():
    ...
    assert False == config_utils.get_boolean_config_item_value(
        config, config_section, config_item, True
    )
    assert False == config_utils.get_boolean_config_item_value(
        config, config_section, config_item, False
...
def test_fetch_ec2_metadata_token_disabled_default_value():
    config = get_config()
    assert False == metadata.fetch_ec2_metadata_token_disabled(config)
...
def test_url_request_helper_does_not_fetch_metadata_token_due_to_token_fetch_disabled_in_config_file(
    ...
    get_aws_ec2_metadata_token_mock = mocker.patch(
        "efs_utils_common.metadata.get_aws_ec2_metadata_token"
    )
    url_open_mock = mocker.patch.object(metadata, "urlopen")
    metadata.url_request_helper(
...
def test_url_request_helper_unauthorized_error(mocker, caplog):
    ...
    resp = metadata.url_request_helper(
        config, efs_utils_common.constants.INSTANCE_METADATA_SERVICE_URL, "", ""
    )
...
def test_get_botocore_client_fips_enabled_use_awsprofile(mocker):
    config = get_config()
    mocker.patch.object(metadata, "get_fips_config", return_value=True)
    mocker.patch("botocore.config.Config", return_value="fips_enabled")
    get_target_region_mock = mocker.patch.object(
        metadata, "get_target_region", return_value=DEFAULT_REGION
    )
    efs_utils_common.constants.BOTOCORE_PRESENT = True
    ...
    client = metadata.get_botocore_client(config, "efs", {"awsprofile": "test_profile"})
...
def test_get_botocore_client_fips_enabled(mocker):
    config = get_config()
    mocker.patch("efs_utils_common.metadata.get_fips_config", return_value=True)
    mocker.patch("botocore.config.Config", return_value="fips_enabled")
    get_target_region_mock = mocker.patch(
        "efs_utils_common.metadata.get_target_region", return_value=DEFAULT_REGION
    )
    efs_utils_common.constants.BOTOCORE_PRESENT = True
    boto_session_mock = MagicMock()
    boto_session_mock.create_client.return_value = "fake-client"
    ...
    client = metadata.get_botocore_client(config, "efs", {})
...
def test_get_botocore_client_use_awsprofile_profile_not_found(mocker, capsys):
    ...
    with pytest.raises(SystemExit) as ex:
        metadata.get_botocore_client(config, "efs", {"awsprofile": "test_profile"})
...
def test_get_assumed_profile_credentials_via_botocore_botocore_not_present(mocker):
    ...
    efs_utils_common.aws_credentials.BOTOCORE_PRESENT = False

    boto_session_mock = MagicMock()
    boto_session_mock.set_config_variable.return_value = None
    mocker.patch("botocore.session.get_session", return_value=boto_session_mock)

    credentials = aws_credentials.botocore_credentials_helper("test_profile")
...
def test_get_assumed_profile_credentials_via_botocore_botocore_present(mocker):
    ...
    efs_utils_common.aws_credentials.BOTOCORE_PRESENT = True
...
def test_get_assumed_profile_credentials_via_botocore_botocore_present_profile_not_found(
    ...
    with pytest.raises(SystemExit) as ex:
        aws_credentials.botocore_credentials_helper("test_profile")
...
def test_get_system_release_version_macos(mocker):
    mocker.patch(
        "efs_utils_common.platform_utils.check_if_platform_is_mac", return_value=True
    )
    platform_mock = mocker.patch("platform.platform", return_value=MACOS)
    assert MACOS == platform_utils.get_system_release_version()
...
def test_get_system_release_version_linux_read_from_sys_release_path(mocker):
    ...
    assert AL2 == platform_utils.get_system_release_version()
...
def test_get_system_release_version_linux_unknown(mocker):
    ...
    platform_mock = mocker.patch("platform.platform")
    assert (
        efs_utils_common.constants.DEFAULT_UNKNOWN_VALUE
        == platform_utils.get_system_release_version()
    )
    utils.assert_not_called(platform_mock)
    utils.assert_called_n_times(open_mock, 2)
...
def test_stunnel5_al2_with_pretty_name(mocker):
    ...
    mocker.patch(
        "efs_utils_common.process_utils.get_system_release_version",
        return_value=efs_utils_common.constants.AMAZON_LINUX_2_PRETTY_NAME,
    )
    process_utils._stunnel_bin()
...
def test_stunnel5_al2_with_release_id(mocker):
    ...
    mocker.patch(
        "efs_utils_common.process_utils.get_system_release_version",
        return_value=efs_utils_common.constants.AMAZON_LINUX_2_RELEASE_ID,
    )
    process_utils._stunnel_bin()
    args, _ = check_output_mock.call_args
    args = args[0]
...
def test_get_ipv6_addresses_success():
    ...
    with patch("socket.getaddrinfo", return_value=mock_addrinfo):
        result = network_utils.get_ipv6_addresses(hostname)
