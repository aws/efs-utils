...
from efs_utils_common.config_utils import get_url_request_timeout
...
def _make_config_with_timeout(value=None):
    ...
    if value is not None:
        config.set(CONFIG_SECTION, URL_REQUEST_TIMEOUT_ITEM, str(value))
    return config
...
def test_get_url_request_timeout_reads_large_value():
    """A large value should be read correctly."""
    config = _make_config_with_timeout("10")
    result = get_url_request_timeout(config)
    assert result == 10.0


def test_get_url_request_timeout_missing_section():
    """When the config section doesn't exist, should fall back to default."""
    try:
        config = ConfigParser.SafeConfigParser()
    except AttributeError:
        config = ConfigParser()
    # Don't add any section
    result = get_url_request_timeout(config)
    assert result == DEFAULT_URL_REQUEST_TIMEOUT_SEC


def test_get_url_request_timeout_custom_section():
    """When a custom config section is specified, it should read from that section."""
    try:
        config = ConfigParser.SafeConfigParser()
    except AttributeError:
        config = ConfigParser()
    config.add_section("mount-watchdog")
    config.set("mount-watchdog", URL_REQUEST_TIMEOUT_ITEM, "0.1")
    result = get_url_request_timeout(config, config_section="mount-watchdog")
    assert result == 0.1
