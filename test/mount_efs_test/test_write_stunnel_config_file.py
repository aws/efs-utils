...
def _get_config(stunnel_debug_enabled=False, stunnel_check_cert_hostname=False, stunnel_check_cert_validity=False):
...
def _get_mount_options(port=PORT, verify=mount_efs.DEFAULT_STUNNEL_VERIFY_LEVEL):
    options = {
        'tlsport': port,
        'verify': verify,
    }
...
def _get_expected_efs_config(port=PORT, dns_name=DNS_NAME, verify=mount_efs.DEFAULT_STUNNEL_VERIFY_LEVEL,
                             check_cert_hostname=False, check_cert_status=False):
...
def test_write_stunnel_config_file(mocker, tmpdir):
    ...
    config_file = mount_efs.write_stunnel_config_file(_get_config(), state_file_dir, FS_ID, MOUNT_POINT, PORT, DNS_NAME,
                                                      _get_mount_options())
...
def test_write_stunnel_config_with_debug(mocker, tmpdir):
    ...
    config_file = mount_efs.write_stunnel_config_file(_get_config(stunnel_debug_enabled=True), state_file_dir, FS_ID,
                                                      MOUNT_POINT, PORT, DNS_NAME, _get_mount_options())
    ca_mocker.assert_called_once()
...
def test_write_stunnel_config_with_verify(mocker, tmpdir):
    ...
    _validate_config(config_file, mount_efs.STUNNEL_GLOBAL_CONFIG, _get_expected_efs_config(verify=0))


def test_write_stunnel_config_with_check_cert_hostname(mocker, tmpdir):
    ca_mocker = mocker.patch('mount_efs.add_stunnel_ca_options')

    config_file = mount_efs.write_stunnel_config_file(_get_config(stunnel_check_cert_hostname=True), str(tmpdir), FS_ID,
                                                      MOUNT_POINT, PORT, DNS_NAME, _get_mount_options())
    ca_mocker.assert_called_once()
...
def test_write_stunnel_config_with_check_cert_status(mocker, tmpdir):
    ...
    config_file = mount_efs.write_stunnel_config_file(_get_config(stunnel_check_cert_validity=True), str(tmpdir), FS_ID,
                                                      MOUNT_POINT, PORT, DNS_NAME, _get_mount_options())
    ca_mocker.assert_called_once()
