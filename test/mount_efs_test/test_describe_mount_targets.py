...
def _test_describe_mount_targets_response(mocker, response, expected_describe_time, desired_mount_targets=None,
                                          desired_exception=None, desired_message=None):
    describe_mount_targets_mock = mocker.patch('mount_efs.efs_describe_mount_targets_helper', side_effect=[response])
...
def test_describe_mount_targets_return_correct(mocker):
    ...
    _test_describe_mount_targets_response(mocker, response, 1, response.get("MountTargets"))
...
def test_describe_mount_targets_failed_file_system_not_found(mocker):
    ...
    _test_describe_mount_targets_response(mocker, ClientError(exception_response, OPERATION_NAME), 1,
                                          desired_exception=mount_efs.FallbackException, desired_message='not found')
...
def test_describe_mount_targets_failed_unknown_client_error(mocker):
    ...
    _test_describe_mount_targets_response(mocker, ClientError(exception_response, OPERATION_NAME), 1,
                                          desired_exception=mount_efs.FallbackException, desired_message='Unexpected error')


def test_describe_mount_targets_failed_unknown_error(mocker):
    _test_describe_mount_targets_response(mocker, Exception('Unknown'), 1,
                                          desired_exception=mount_efs.FallbackException, desired_message='Unknown error')


def test_describe_mount_targets_failed_endpoint_error(mocker):
    _test_describe_mount_targets_response(mocker, EndpointConnectionError(endpoint_url='https://efs.us-east-1.com'), 1,
                                          desired_exception=mount_efs.FallbackException,
...
def test_describe_availability_zones_dryrun_failed_no_credential_error(mocker):
    _test_describe_mount_targets_response(mocker, NoCredentialsError(), 1,
                                          desired_exception=mount_efs.FallbackException,
                                          desired_message='please confirm your aws credentials are properly configured')


def _test_get_mount_target_in_az(mocker, get_mount_targets_info_response, desired_describe_time, desired_get_az_id_time,
                                 desired_mount_target=None, az_id=AZ_ID, az_name=AZ_NAME, desired_exception=None,
                                 desired_message=None, on_premise=False):
    get_mount_targets_info_mock = mocker.patch('mount_efs.get_mount_targets_info', side_effect=[get_mount_targets_info_response])
    get_az_id_by_az_name_mock = mocker.patch('mount_efs.get_az_id_by_az_name', return_value=az_id)

    if desired_exception:
        assert desired_message is not None
        with pytest.raises(mount_efs.FallbackException) as excinfo:
            mount_efs.get_mount_target_in_az(MOCK_EFS_AGENT, MOCK_EC2_AGENT, FS_ID, az_name)
        assert desired_message in str(excinfo)
    else:
        mount_target = mount_efs.get_mount_target_in_az(MOCK_EFS_AGENT, MOCK_EC2_AGENT, FS_ID, az_name)
...
def test_get_mount_target_in_az_with_match_az_id(mocker):
    ...
    _test_get_mount_target_in_az(mocker, response, desired_describe_time=1, desired_get_az_id_time=1,
                                 desired_mount_target=MOUNT_TARGET_INFO, az_id=AZ_ID)
...
def test_get_mount_target_in_az_no_mount_targets(mocker):
    ...
    _test_get_mount_target_in_az(mocker, get_mount_targets_info_response, desired_describe_time=1, desired_get_az_id_time=0,
                                 desired_exception=mount_efs.FallbackException, desired_message='Cannot find mount target')
...
def test_get_mount_target_in_az_no_available_mount_targets(mocker):
    ...
    _test_get_mount_target_in_az(mocker, get_mount_targets_info_response, desired_describe_time=1, desired_get_az_id_time=0,
                                 desired_exception=mount_efs.FallbackException, desired_message='is in available state yet')
...
def test_get_mount_target_in_az_no_az_id_match_to_az_name(mocker):
    ...
    _test_get_mount_target_in_az(mocker, get_mount_targets_info_response, desired_describe_time=1,
                                 desired_get_az_id_time=1, az_id=None,
                                 desired_exception=mount_efs.FallbackException, desired_message='No matching az id')
...
def test_get_mount_target_in_az_random_pick_mount_target_for_on_premise(mocker):
    ...
    _test_get_mount_target_in_az(mocker, get_mount_targets_info_response, desired_describe_time=1,
                                 desired_get_az_id_time=0, az_name=None, on_premise=True)
...
def test_get_mount_target_in_az_no_mount_target_in_az(mocker):
    ...
    _test_get_mount_target_in_az(mocker, get_mount_targets_info_response,
                                 desired_describe_time=1, desired_get_az_id_time=1,
                                 desired_exception=mount_efs.FallbackException,
...
def test_get_mount_target_in_az_efs_agent_is_none(mocker):
    _test_get_mount_target_in_az_agent_is_none(mocker, efs_agent=MOCK_EFS_AGENT, ec2_agent=None)


def test_get_mount_target_in_az_ec2_agent_is_none(mocker):
    _test_get_mount_target_in_az_agent_is_none(mocker, efs_agent=None, ec2_agent=MOCK_EC2_AGENT)
