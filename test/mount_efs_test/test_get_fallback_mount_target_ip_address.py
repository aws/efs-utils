...
def _get_mock_config(config_section='mount', has_fallback_to_mount_target_ip_address_item=True,
...
def test_fall_back_to_mount_target_ip_address_not_enabled_in_config_file():
    config = _get_mock_config(has_fallback_to_mount_target_ip_address_item=True, fallback_to_mount_target_ip_address=False)
    assert False == mount_efs.check_if_fall_back_to_mount_target_ip_address_is_enabled(config)


def test_fall_back_to_mount_target_ip_address_enabled_in_config_file():
    config = _get_mock_config(has_fallback_to_mount_target_ip_address_item=True, fallback_to_mount_target_ip_address=True)
    assert True == mount_efs.check_if_fall_back_to_mount_target_ip_address_is_enabled(config)
...
def test_check_if_ip_can_be_resolved_ip_connect_to_mount_target_timeout_first_time(mocker):
    ip_mock = mocker.patch('socket.create_connection', side_effect=[socket.timeout,
                                                                    socket.socket(socket.AF_INET, socket.SOCK_STREAM)])
    assert True == mount_efs.mount_target_ip_address_can_be_resolved(FALLBACK_IP_ADDRESS)
...
def test_get_fallback_mount_target_ip_address_throw_fallback_exception(mocker):
    ...
    get_target_az_mock = mocker.patch('mount_efs.get_target_az', return_value=DEFAULT_AZ)
    get_botocore_client_mock = mocker.patch('mount_efs.get_botocore_client', side_effect=[MOCK_EFS_AGENT, MOCK_EC2_AGENT])
    get_mount_target_az_mock = mocker.patch('mount_efs.get_mount_target_in_az',
...
def test_get_fallback_mount_target_ip_address_mount_target_ip(mocker):
    ...
    get_target_az_mock = mocker.patch('mount_efs.get_target_az', return_value=DEFAULT_AZ)
    get_botocore_client_mock = mocker.patch('mount_efs.get_botocore_client', side_effect=[None, None])
    get_mount_target_mock = mocker.patch('mount_efs.get_mount_target_in_az', return_value={'IpAddress': FALLBACK_IP_ADDRESS})

    assert FALLBACK_IP_ADDRESS == mount_efs.get_fallback_mount_target_ip_address_helper(config, {}, FS_ID)
...
def test_get_fall_back_ip_address_success(mocker):
    ...
    check_fallback_enabled_mock = mocker.patch('mount_efs.check_if_fall_back_to_mount_target_ip_address_is_enabled',
                                               return_value=True)
    get_fallback_mount_target_ip_mock = mocker.patch('mount_efs.get_fallback_mount_target_ip_address_helper',
                                                     return_value=FALLBACK_IP_ADDRESS)
    check_ip_resolve_mock = mocker.patch('mount_efs.mount_target_ip_address_can_be_resolved', return_value=True)

    ip_address = mount_efs.get_fallback_mount_target_ip_address(config, FS_ID, DEFAULT_NFS_OPTIONS, DNS_NAME)
...
def test_get_fall_back_ip_address_feature_not_enabled(mocker):
    ...
    check_fallback_enabled_mock = mocker.patch('mount_efs.check_if_fall_back_to_mount_target_ip_address_is_enabled',
                                               return_value=False)
    get_fallback_mount_target_ip_mock = mocker.patch('mount_efs.get_fallback_mount_target_ip_address_helper')
    check_ip_resolve_mock = mocker.patch('mount_efs.mount_target_ip_address_can_be_resolved')

    with pytest.raises(mount_efs.FallbackException) as excinfo:
        mount_efs.get_fallback_mount_target_ip_address(config, FS_ID, DEFAULT_NFS_OPTIONS, DNS_NAME)
