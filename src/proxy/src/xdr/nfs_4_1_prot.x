...
/* As a part of AWS File Service protocol (NFS4 extension), we
 * introduced the "extended NFS4 attributes (rfc8276)" to the
 * protocol. This should be built as a part of the overall AWS
 * file service protocol in "awsfile_prot.x"
 */
#include "xattr_prot.x"
...
const NFS4_INT64_MAX            = 2147483647;
const NFS4_UINT64_MAX           = 4294967295;
const NFS4_INT32_MAX            = 2147483647;
const NFS4_UINT32_MAX           = 4294967295;

const NFS4_MAXFILELEN           = 4294967295;
const NFS4_MAXFILEOFF           = 4294967294;
...
enum nfsstat4 {
 ...
 /* NFS4 Extended Attributes (rfc8276) */
 NFS4ERR_NOXATTR        = 10095,/* xattr does not exist    */
 NFS4ERR_XATTR2BIG      = 10096, /* xattr value is too big  */

 /* AWSFILE Extended errors*/
 NFS4ERR_AWSFILE_BYPASS = 100001 /* NFS error for bypassing */
};
...
const ACE4_DELETE               = 16384;
const ACE4_READ_ACL             = 32768;
const ACE4_WRITE_ACL            = 65536;
const ACE4_WRITE_OWNER          = 131072;
const ACE4_SYNCHRONIZE          = 262144;
...
const ACE4_GENERIC_READ = 262145;
 ...
 *      ACE4_SYNCHRONIZE
 */
const ACE4_GENERIC_WRITE = 262161;
 ...
 *      ACE4_SYNCHRONIZE
 */
const ACE4_GENERIC_EXECUTE = 262176;
 ...
 */
const MODE4_SUID = 2048;  /* set user id on execution */
const MODE4_SGID = 1024;  /* set group id on execution */
const MODE4_SVTX = 512;  /* save text even after use */
const MODE4_RUSR = 256;  /* read permission: owner */
const MODE4_WUSR = 128;  /* write permission: owner */
const MODE4_XUSR = 64;  /* execute permission: owner */
const MODE4_RGRP = 32;  /* read permission: group */
const MODE4_WGRP = 16;  /* write permission: group */
const MODE4_XGRP = 8;  /* execute permission: group */
const MODE4_ROTH = 4;  /* read permission: other */
const MODE4_WOTH = 2;  /* write permission: other */
const MODE4_XOTH = 1;  /* execute permission: other */
...
const RET4_DURATION_INFINITE    = 4294967295;
...
union locker4 switch (boolflag new_lock_owner) {
 case TRUE:
...
/* Arguments exactly the same as READ4args */
typedef READ4args AWSFILE_READ_BYPASS4args;
...
union secinfo4 switch (auth_flavor4 flavor) {
 case RPCSEC_GSS:
...
union callback_sec_parms4 switch (auth_flavor4 cb_secflavor) {
case AUTH_NONE:
        void;
case AUTH_SYS:
...
const EXCHGID4_FLAG_MASK_PNFS           = 12582912;
...
union newtime4 switch (boolflag nt_timechanged) {
case TRUE:
        nfstime4           nt_time;
case FALSE:
        void;
...
union newoffset4 switch (boolflag no_newoffset) {
case TRUE:
        offset4           no_offset;
case FALSE:
        void;
...
union newsize4 switch (boolflag ns_sizechanged) {
case TRUE:
        length4         ns_size;
case FALSE:
        void;
...
union layoutreturn_stateid switch (boolflag lrs_present) {
case TRUE:
...
/* NFS4 Extended Attributes (rfc8276) */
 ...
 /* AWS File read bypass operations */
 OP_AWSFILE_READ_BYPASS = 200001
};
...
union nfs_argop4 switch (nfs_opnum4 argop) {
 ...
 /* Operations introduced for NFS4 extended attributes (rfc8276) */
 ...
 /* AWS File read bypass operations */
 case OP_AWSFILE_READ_BYPASS:
                        AWSFILE_READ_BYPASS4args read_bypass;
};
...
union nfs_resop4 switch (nfs_opnum4 resop){
 ...
 /* AWS File read bypass operations */
 case OP_AWSFILE_READ_BYPASS:
                        AWSFILE_READ_BYPASS4res read_bypass;
};
...
/*
 * new operations for AWS File Services NFS Extension
 */

struct CB_AWSFILE_HEARTBEAT4args {
        clientid4           clientid;
};

struct CB_AWSFILE_HEARTBEAT4res {
        nfsstat4            hb_status;
};
...
/* new callback operations for AWS File Services NFS Extension */
        OP_CB_AWSFILE_HEARTBEAT     = 100001
};
...
/* new callback operations for AWS File Services NFS Extension */
 case OP_CB_AWSFILE_HEARTBEAT:
      CB_AWSFILE_HEARTBEAT4args     opcbawsfile_heartbeat;
};
...
union nfs_cb_resop4 switch (nfs_cb_opnum4 resop){
 ...
 /* new callback operations for AWS File Services NFS Extension */
 case OP_CB_AWSFILE_HEARTBEAT:
                        CB_AWSFILE_HEARTBEAT4res opcbawsfile_heartbeat;
};
...
struct awsfile_bypass_data_locator {
        opaque          bucket_name<>;
        opaque          s3_key<>;
        opaque          etag<>;
        offset4         offset; /* read start offset */ 
        count4          count;  /* size of read in bytes */
};

struct AWSFILE_READ_BYPASS4resok {
        nfs_fh4                         filehandle;
        awsfile_bypass_data_locator     data_locator;
        // file size in bytes, needed to calculate readahead EOF
        uint64_t                        file_size;
};

union AWSFILE_READ_BYPASS4res switch (nfsstat4 status) {
 case NFS4_OK:
        READ4resok                      resok;
 case NFS4ERR_AWSFILE_BYPASS: /* New NFS error for bypassing */ 
        AWSFILE_READ_BYPASS4resok        bypass;
 default:
        void;
};

#if INCLUDE_NFS4_PROGRAMS
